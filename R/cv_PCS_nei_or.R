...
###############################################################################
###############################################################################
##### PCS k-fold cross-validation for Nodewise regression using the 'OR' rule
###############################################################################
###############################################################################
###############################################################################

# k-folds CV for PCS: Nodewise regression 'OR' 

# Function that estimates the threshold parameter in Partial Correlation Screening when the GMM is estimated using Nodewise regression using the 'OR' rule
...
# lambda.opt is the regularization parameter to estimate the GGM using Nodewise regression and the 'OR' rule
# fold is the number of folds

# The function returns the threhsold parameter that minimizes the mean squared error

cv_PCS_nei_or = function(x,lambda.opt,fold){
...
x = scale(x) 
 ...
 loss.cv = function(x.train,x.test,lambda.opt,taulist){
  list.nei.alpha = neigh_or_lambda(x.train,lambda.opt)
