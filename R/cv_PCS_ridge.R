...
############################################################
############################################################
##### PCS k-fold cross-validation for Ridge regression
############################################################
############################################################
############################################################

# k-folds CV for PCS: Ridge regression 

# Function that estimates the threshold parameter in Partial Correlation Screening when the GMM is estimated using Ridge regression
...
# lambda.opt is the regularization parameter to estimate the GGM using Ridge regression
# fold is the number of folds

# The function returns the threhsold parameter that minimizes the mean squared error

cv_PCS_ridge = function(x,lambda.opt,fold){
...
x = scale(x) 
 ...
 loss.cv = function(x.train,x.test,lambda.opt,taulist){
  list.ridge = ridge_lambda(x.train,lambda.opt)
  R = -cov2cor(list.ridge[[2]])
  diag(R) = 1  
