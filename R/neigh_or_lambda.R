...
###########################################################
###########################################################
##### GGM estimation using Nodewise Regression 'OR' Rule
###########################################################
###########################################################
###########################################################

# Estimate the GGM using Nodewise regression + OR rule

# Function that estimates the Regression Weights and the Precision Matrix using nodewise regressions and the OR rule
...
neigh_or_lambda = function(x,lambda){
...
for (i in 1:p){
for (j in 1:p){
Omega[i,j] = max(abs(Omega[i,j]),abs(Omega[j,i]))
Omega[j,i] = Omega[i,j] 
}}
